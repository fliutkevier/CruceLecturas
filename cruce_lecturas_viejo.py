...
def procesar_cruce(ruta_bajada, texto_polizas, verificadores=None, hoy=None,
    ...
    for fila in it:
        ...
        cat = _texto(fila[idx[COL_CATEGORIA]].value) if COL_CATEGORIA in idx else ""
        if not cat:
            posible, _ = separar_error_detalle(obs_limpia, errores, con_codigo)
            reconocido = any(obs_limpia.upper().startswith(e.upper())
                             for e in list(errores) + list(con_codigo))
            if reconocido:
                cat = posible
            else:
                sin_categoria += 1
                sin_reconocer.append(obs_limpia)
        ...
        calc = {
            ...
            "@DETALLE": obs_limpia,
            "@CATEGORIA": cat.upper() if cat else None,
        }
...
def lanzar_ui():
    ...
    def ejecutar_vis():
        ...
        try:
            headers, filas, st = procesar_cruce(estado["bajada"], txt.get("1.0", "end"), elegidos)
        except Exception as exc:
